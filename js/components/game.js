class Game extends React.Component {
  ...
  update(room) {
    ...
    if (this.props.arrowState === WumpusManager.constants.arrowState.armed) {
      ...
      this.print(
        "تیر آماده شلیک است!",
        `روی اون جهتی که میخوایی کلیک کن.`,
        "black",
        WumpusManager.constants.question,
        -2
      );
    } else if (
      this.props.arrowState === WumpusManager.constants.arrowState.fired
    ) {
      ...
      this.print(
        ...
        `ولی به هدفی نخور.`,
        "black",
        WumpusManager.constants.arrow,
        -2
      );
    } else if (
      this.props.arrowState === WumpusManager.constants.arrowState.kill
    ) {
      console.log("You shoot! You hear a thump on the ground.");
      this.print(
        "تو تیر زدی!",
        `صدای جیغ ومپوس شنیده شد`,
        "black",
        WumpusManager.constants.arrow,
        -2
      );
    } else if (
      this.props.arrowState === WumpusManager.constants.arrowState.none
    ) {
      this.print();
    }
    ...
    if (room) {
      ...
      if (room.includes(WumpusManager.constants.gold)) {
        ...
        console.log(
          `You found the treasure in ${this.state.moves} moves! Score: ${score}`
        );
        this.print(
          "تو بردی!",
          `تو طلا را در ${this.state.moves} حرکت پیدا کردی. امتیاز: ${score}`,
          "gold",
          WumpusManager.constants.gold,
          0,
          "alert-warning"
        );
        ...
        this.print(
          "تو باختی!",
          "تو توسط ومپوس خورده شدی!",
          "red",
          WumpusManager.constants.wumpus,
          -5,
          "alert-danger"
        );
        ...
        this.print(
          "تو باختی!",
          "تو داخل چاله سیاه افتادی!",
          "black",
          WumpusManager.constants.crossbones,
          -2,
          "alert-danger"
        );
    ...
    if (!this.state.gameOver) {
      ...
      if (this.state.gameOver) {
        ...
        if (
          ...
          this.props.arrowState === WumpusManager.constants.arrowState.armed
        ) {
          ...
          if (arrowState === WumpusManager.constants.arrowState.kill) {
            const index = dungeon.map[this.state.dungeon.wumpus.y][
              this.state.dungeon.wumpus.x
            ].indexOf(WumpusManager.constants.wumpus);
            dungeon.map[this.state.dungeon.wumpus.y][
              this.state.dungeon.wumpus.x
            ].splice(index);
          }
        ...
        this.setState(
          ...
          () => {
            if (
              !this.update(
                this.state.dungeon.map[playerLocation.y][playerLocation.x]
              )
            ) {
              // Game over.
  ...
  updateAI(isGameOver) {
    ...
    this.state.bestMove &&
      this.grid.current.setValue(
        this.state.bestMove.x,
        this.state.bestMove.y,
        null
      );

    if (!isGameOver) {
      // Show hint for best move.
      const bestMove = AiManager.update(
        this.state.x,
        this.state.y,
        this.state.dungeon.map[this.state.y][this.state.x]
      );

      // Show hint for AI path, if one exists.
      this.oldPath &&
        this.oldPath.forEach((room) => {
  ...
  shootArrow(player, direction) {
    this.setState(
      {
        arrow: this.renderEntity(
          player.x,
          player.y,
          `arrow fas fa-arrow-${direction}`
        ),
      },
      () => {
        setTimeout(() => {
    ...
    return direction ===
      GameManager.direction(player, this.state.dungeon.wumpus)
  ...
  print(
    ...
    text,
    color = "black",
    icon = WumpusManager.constants.clear,
    offset = 0,
    className = null
  ) {
    const message = title ? (
      <div
        class={`mt-1 pl-2 alert ${className} show`}
        role="alert"
        style={{ width: "400px" }}
      >
        <div style={{ float: "left" }}>
          <i
            class={`${WumpusManager.icon(icon)} mr-2`}
  ...
  renderPlayer(x, y, map) {
    ...
    // Render the player and percept indicators (go through each percept found in this cell and render the corresponding icon and color).<FontAwesomeIcon icon="fa-solid fa-robot-astromech" />
    return (
      <Entity
        width="50"
        height="50"
        x={x}
        y={y}
        cellStyle={`player fas fa fa-robot ${
          this.state.gameOver ? "fade" : ""
        }`}
        color="darkgreen"
      >
        {!this.state.gameOver && (
          ...
          if (entity === WumpusManager.constants.pit) {
            objects.push(
              this.renderEntity(
                x,
                y,
                `anchor ${WumpusManager.icon(
                  WumpusManager.constants.pittile
                )} ${this.props.cheatMode ? "" : "d-none"}`,
                "black"
              )
            );
            ...
            objects.push(
              this.renderEntity(
                x,
                y,
                `anchor ${WumpusManager.icon(WumpusManager.constants.wumpus)} ${
                  this.props.cheatMode ? "" : "d-none"
                }`,
                "red"
              )
            ...
            objects.push(
              this.renderEntity(
                x,
                y,
                `anchor ${WumpusManager.icon(WumpusManager.constants.gold)} ${
                  this.props.cheatMode ? "" : "d-none"
                }`,
                "gold"
              )
  ...
  render() {
    ...
    const entities = [
      this.renderPlayer(this.state.x, this.state.y, this.state.dungeon.map),
    ].concat(this.renderObjects(this.state.dungeon.map));
